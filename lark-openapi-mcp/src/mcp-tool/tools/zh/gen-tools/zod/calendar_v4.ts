...
export const calendarV4CalendarEventAttendeeBatchDelete = {
  ...
  schema: {
          ...
          .array(
            z.object({
              type: z
                ...
                .describe(
                  '参与人类型。 Options:user(用户),chat(群组),resource(会议室),third_party(ThirdParty 外部邮箱)',
                )
                .optional(),
              user_id: z
                .string()
    ...
    params: z
      ...
      .optional(),
    path: z.object({
      calendar_id: z.string().describe('日程对应的日历 ID。了解更多，参见[日历 ID 说明]'),
      event_id: z
        .string()
        .describe(
...
export const calendarV4CalendarEventAttendeeChatMemberList = {
  ...
  schema: {
    params: z
      ...
      .optional(),
    path: z.object({
      calendar_id: z.string().describe('日程所在的日历 ID。关于日历 ID 可参见[日历 ID 说明]'),
      event_id: z
        .string()
        .describe(
          '日程 ID。创建日程时会返回日程 ID。你也可以调用以下接口获取某一日历的 ID。- [获取日程列表]- [搜索日程]',
...
export const calendarV4CalendarEventDelete = {
  ...
  schema: {
    params: z
      ...
      .optional(),
    path: z.object({
      calendar_id: z.string().describe('日程所在的日历 ID。了解更多，参见[日历 ID 说明]'),
      event_id: z.string().describe('日程 ID。获取方式：- [创建日程]会返回日程 ID- [获取日程列表]- [搜索日程]'),
    }),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
};
...
export const calendarV4CalendarEventInstanceView = {
  ...
  schema: {
    ...
    path: z.object({ calendar_id: z.string().describe('日历 ID。关于日历 ID 可参见[日历 ID 说明]') }),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
};
export const calendarV4CalendarEventInstances = {
  ...
  schema: {
    ...
    path: z.object({
      calendar_id: z.string().describe('日历 ID。关于日历 ID 可参见[日历 ID 说明]'),
      event_id: z
        .string()
        .describe(
          '日程 ID。创建日程时会返回日程 ID。你也可以调用以下接口获取某一日历的 ID。- [获取日程列表]- [搜索日程]',
...
export const calendarV4CalendarEventMeetingMinuteCreate = {
  ...
  schema: {
    path: z.object({
      calendar_id: z.string().describe('日历 ID。调用[查询主日历信息]接口，获取当前身份的主日历 ID'),
      event_id: z.string().describe('日程 ID。调用[搜索日程]接口，在指定日历内搜索日程并获取日程 ID'),
    }),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
};
  ...
  schema: {
            ...
            meeting_settings: z
              .object({
                ...
                auto_record: z
                  .boolean()
                  .describe(
                    '是否开启自动录制。**可选值有**：- true：开启- false：不开启**默认值**：空，表示不更新该字段',
                  )
                  .optional(),
                open_lobby: z
                  .boolean()
    ...
    path: z.object({
      calendar_id: z
        .string()
        .describe(
          '日程所在的日历 ID。ID 获取方式：- [创建共享日历]时，接口会返回日历 ID。- 调用[查询主日历信息]、[查询日历列表]或者[搜索日历]接口，获取所需的日历 ID。-',
        ),
      event_id: z
        .string()
        .describe(
          '日程 ID。ID 获取方式：- [创建日程]时，接口会返回日程 ID。- 调用[获取日程列表]或者[搜索日程]接口，获取所需的日程 ID',
        ),
    }),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
...
export const calendarV4SettingGenerateCaldavConf = {
  ...
  schema: {
    data: z
      .object({ device_name: z.string().describe('需要同步日历的设备名，在日历中用来展示。**默认值**：空').optional() })
      .optional(),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
};
