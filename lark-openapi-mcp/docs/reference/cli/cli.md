...
```bash
# Basic login
npx -y @ticoag/lark-mcp login -a cli_xxxx -s your_secret

# Login with specific OAuth scope
npx -y @ticoag/lark-mcp login -a cli_xxxx -s your_secret --scope offline_access docx:document

# Login with custom domain (for Lark international)
npx -y @ticoag/lark-mcp login -a cli_xxxx -s your_secret -d https://open.larksuite.com
```
...
npx -y @ticoag/lark-mcp logout -a cli_xxxx

# Clear tokens for all apps
npx -y @ticoag/lark-mcp logout
```
...
| `--app-secret` | `-s` | Feishu/Lark application App Secret | `-s xxxx` |
| `--domain` | `-d` | Feishu/Lark API domain, default is https://open.feishu.cn | `-d https://open.larksuite.com` |
| `--tools` | `-t` | List of API tools to enable, separated by spaces or commas; when omitted, all supported tools are enabled | `-t im.v1.message.create,im.v1.chat.create` |
